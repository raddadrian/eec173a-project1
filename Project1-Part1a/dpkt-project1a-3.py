...
def parse_pcap(pcap_file):
    # IP address to filter for http://httpforever.com
    HTTPFOREVER_IP = "146.190.62.39"

    with open(pcap_file, 'rb') as f:
        pcap = dpkt.pcap.Reader(f)

        # intialize count vars for HTTP/HTTPS
        httpCounts = 0
        httpsCounts = 0
        ...
        for timestamp, data in pcap:
            ...
            if isinstance(eth.data, dpkt.ip.IP):
                ...
                if src_IP != HTTPFOREVER_IP and dst_IP != HTTPFOREVER_IP:
                    continue
